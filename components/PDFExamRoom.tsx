...
//
// CHANGELOG:
// - FIX mergeAnswers: lưu null cho ý TF chưa chọn (thay vì false)
//   → Tránh bug học sinh để trống ý c,d vô tình được tính là "chọn Sai đúng"
//   → scoringService.ts sẽ đọc null = chưa trả lời → tính sai (không được điểm)
...
type TFAnswers = { [n: number]: string[] }; // '' | 'Đ' | 'S' cho từng ý a,b,c,d
type SAAnswers = { [n: number]: string };

/**
 * Gộp tất cả đáp án thành 1 object để nộp bài.
 *
 * TF FIX: Dùng null cho ý chưa chọn thay vì false.
 *   - v[i] === 'Đ' → true
 *   - v[i] === 'S' → false
 *   - v[i] === ''  → null  (chưa chọn — scoringService nhận null → tính sai)
 *
 * Điều này giúp scoringService phân biệt:
 *   "học sinh chọn Sai" vs "học sinh chưa trả lời"
 */
function mergeAnswers(mc: MCAnswers, tf: TFAnswers, sa: SAAnswers): { [n: number]: string } {
  ...
  // True/False — ✅ FIX: null cho ý chưa chọn
  Object.entries(tf).forEach(([k, v]) => {
    ...
    if (hasAny) {
      ...
      ['a', 'b', 'c', 'd'].forEach((lbl, i) => {
        if (v[i] === 'Đ') {
          obj[lbl] = true;        // chọn Đúng
        } else if (v[i] === 'S') {
          obj[lbl] = false;       // chọn Sai
        } else {
          obj[lbl] = null;        // chưa chọn → scoringService tính là sai
        }
      });
...
const PDFExamRoom: React.FC<PDFExamRoomProps> = ({
  ...
  const setTF = (qNum: number, idx: number, val: string) =>
    setTfAnswers((p) => {
      const cur  = p[qNum] || ['', '', '', ''];
      const next = [...cur];
      // Toggle: bấm lại cùng ký tự → bỏ chọn (về '')
  ...
  const handleSubmit = async (auto = false) => {
    ...
    if (!submissionId) {
      alert('Lỗi phiên thi. Vui lòng thử lại.');
      return;
    }
    ...
    try {
      ...
      const submission = await submitExam(submissionId, merged, exam, {
        tabSwitchCount,
  ...
  return (
    <div className="h-screen flex flex-col bg-gray-100 overflow-hidden">
      ...
      <div className="bg-white border-b border-gray-200 px-3 py-2 flex items-center gap-2 shrink-0 z-10 shadow-sm">
        ...
        {/* Timer */}
        <div
          className={`font-mono font-bold px-2.5 py-1.5 rounded-lg text-sm shrink-0
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 max-w-sm w-full shadow-2xl">
            ...
            <div className="text-sm text-gray-600 space-y-1 mb-4">
              <p>
                Bạn đã trả lời: <strong>{totalAnswered}/{totalQ}</strong> câu
              </p>
              {totalAnswered < totalQ && (
                <p className="text-orange-600">⚠ Còn {totalQ - totalAnswered} câu chưa làm</p>
              )}
              <p>
                Thời gian còn lại: <strong>{formatTimer(timeLeft)}</strong>
              </p>
            </div>
