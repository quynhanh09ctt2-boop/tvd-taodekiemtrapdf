...
const StudentPortal: React.FC<StudentPortalProps> = ({ onJoinRoom, onBack }) => {
  ...
  const handleJoinRoomWithAccount = async () => {
    ...
    try {
      ...
      // ✅ FIX: dùng Firebase auth UID làm student.id
      // PDFExamRoom/ExamRoom sẽ ghi submission với student.id = auth.uid
      // nên lookup cũng phải dùng auth.uid để tìm đúng bài đã nộp
      ...
      const existingSubmission = await getStudentSubmission(room.id, effectiveId);
