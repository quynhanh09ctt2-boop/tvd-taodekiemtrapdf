...
const StudentPortal: React.FC<StudentPortalProps> = ({ onJoinRoom, onBack }) => {
  ...
  const handleJoinRoomWithAccount = async () => {
    ...
    try {
      ...
      if (room.status === 'closed')                          { alert('❌ Phòng làm bài đã đóng!'); return; }
      if (room.status === 'waiting' && !room.allowLateJoin)  { alert('❌ Phòng làm bài chưa bắt đầu!'); return; }
  ...
  const handleJoinRoomAnonymous = async () => {
    ...
    try {
      ...
      const now = Date.now();
      if (room.opensAt && now < new Date(room.opensAt).getTime()) {
        alert(`⏳ Phòng làm bài chưa mở!\nSẽ mở lúc: ${new Date(room.opensAt).toLocaleString('vi-VN')}`);
        return;
      }
      if (room.closesAt && now >= new Date(room.closesAt).getTime()) {
        alert(`⛔ Phòng làm bài đã hết giờ!\nĐã đóng lúc: ${new Date(room.closesAt).toLocaleString('vi-VN')}`);
        return;
      }
      ...
      >
        <div className="max-w-lg w-full">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-teal-600">Chọn cách vào phòng làm bài</p>
          </div>
          ...
          <div className="space-y-3">
            ...
            >
              ...
              <div className="flex-1">
                <h2 className="text-lg font-bold text-gray-900">Làm bài tự do</h2>
          ...
          <p className="text-center text-teal-600 mt-6 text-xs">
            💡 "Làm bài tự do" chỉ khả dụng nếu giáo viên bật tính năng này
          </p>
        </div>
      </div>
    );
        ...
        <div className="max-w-md w-full">
          <div className="bg-white rounded-3xl shadow-2xl p-8">
            <div className="text-center mb-6">
              ...
              <h1 className="text-2xl font-bold text-gray-900">Làm bài tự do</h1>
          ...
          <div className="bg-white rounded-2xl shadow-2xl p-8">
            ...
            <div className="space-y-3 mb-6">
              {[
                ...
                'Vào làm bài trực tiếp từ danh sách phòng',
              ].map((step, i) => (
