...
export const signInWithUsernamePassword = async (username: string, password: string): Promise<User | null> => {
  try {
    const q = query(
      collection(db, 'users'),
    ...
    // Sign in anonymously to have a Firebase Auth session
    await signInAnonymously(auth);

    return user;
  } catch (error) {
    console.error('Username/Password sign in error:', error);
    throw error;
  }
};

export const getStudentFromStorage = (): User | null => {
...
export const getCurrentUser = async (): Promise<User | null> => {
  // Check storage first (for students)
  const student = getStudentFromStorage();
  if (student) {
    // Ensure we have an anonymous session if we are a student from storage
    if (!auth.currentUser) {
      await signInAnonymously(auth);
    }
    return student;
...
export const importStudentsFromExcel = async (file: File, classId: string): Promise<{ success: number; errors: string[] }> => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        for (const row of jsonData) {
          const name = row['Họ tên'] || row['Name'] || row['name'] || row['Full Name'];
          const username = row['Tài khoản'] || row['Username'] || row['username'] || row['ID'];
          const password = row['Mật khẩu'] || row['Password'] || row['password'] || row['Pass'];

          if (!name || !username || !password) {
            errors.push(`Dòng thiếu thông tin (Họ tên, Tài khoản, Mật khẩu): ${JSON.stringify(row)}`);
            continue;
          }
          ...
          const q = query(collection(db, 'users'), where('username', '==', usernameStr));
          const snap = await getDocs(q);
          if (!snap.empty) {
            errors.push(`Tài khoản ${usernameStr} đã tồn tại`);
            continue;
          }
...
export const updateSubmission = async (submissionId: string, data: Partial<Submission>): Promise<void> => {
  await updateDoc(doc(db, 'submissions', submissionId), data as Record<string, any>);
};

// ============ RESET SYSTEM (Admin only) ============

export const resetSystemData = async (keepAdminId: string): Promise<void> => {
  ...
  for (const collName of collectionsToClear) {
    ...
    if (!snap.empty) {
      ...
      snap.docs.forEach(d => batch.delete(d.ref));
      await batch.commit();
      console.log(`🗑️ Cleared collection: ${collName}`);
    }
  }
  ...
  if (deletedUsers > 0) {
    await batch.commit();
    console.log(`🗑️ Cleared ${deletedUsers} users (kept admin ${keepAdminId})`);
  }
};
